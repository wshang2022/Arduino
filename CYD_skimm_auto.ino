...
#include "SD.h"
#include "SPI.h"
#define USE_PORTRAIT  // Comment this out to switch to LANDSCAPE
...
void doScan() {
  int maxBarWidth = SCREEN_W - 40;  // Dynamic bar width
  ...
  for (int j = 0; j < results->getCount(); j++) {
    ...
    // Calculate width to fit the screen (Screen Width minus 20px total margin)
    int bleMaxBarW = SCREEN_W - 20;

    // Update barWidth calculation to use the same max
    int barWidth = constrain(map(rssi, -90, -30, 0, bleMaxBarW), 0, bleMaxBarW);

    // Draw using 10px as the left starting point
    tft.drawRect(10, y, bleMaxBarW, 6, TFT_DARKGREY);
    tft.fillRect(10, y, barWidth, 6, (rssi > -50) ? TFT_RED : TFT_GREEN);

    // Move cursor down for the next entry
  ...
  for (int i = 0; i < bleResults->getCount(); i++) {
    ...
    int barWidth = map(rssi, -90, -30, 0, maxBarWidth);
    barWidth = constrain(barWidth, 0, maxBarWidth);
    int y = tft.getCursorY();
    // Calculate width to fit the screen (Screen Width minus 20px total margin)
    int bleMaxBarW = SCREEN_W - 20;
    // Update barWidth calculation to use the same max
    barWidth = constrain(map(rssi, -90, -30, 0, bleMaxBarW), 0, bleMaxBarW);
    // Draw using 10px as the left starting point
    tft.drawRect(10, y, bleMaxBarW, 6, TFT_DARKGREY);
    tft.fillRect(10, y, barWidth, 6, (rssi > -50) ? TFT_RED : TFT_GREEN);
    // Move cursor down for the next entry
...
void drawBattery() {
  ...
  // The 320 and 420 represent the minimum and maximum millivolts of a Lithium-Polymer battery:
  // 320 : 3.2V(Empty), 420 : 4.2V(Full)
  int percent = constrain(map(v * 100, 320, 420, 0, 100), 0, 100);
  int bx = SCREEN_W - 40;  // Always 40px from the right edge
  int by = 5;

  tft.fillRect(bx - 45, by, 85, 15, TFT_BLACK);
  tft.drawRect(bx, by, 30, 12, TFT_WHITE);
  tft.fillRect(bx + 30, by + 3, 3, 6, TFT_WHITE);

  // Bar and color math...
  tft.setCursor(bx - 38, by + 2);
