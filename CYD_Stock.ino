...
void drawDashboard() {
  ...
  tft.drawLine(0, 27, 320, 27, GRID_COLOR);
  
  // Calculate row height - up to 8 stocks
  int rowHeight = 26;
  int startY = 30;
  int maxRows = min(TOTAL_STOCKS, 8);
  
  // Draw each stock as a row
  for(int i = 0; i < maxRows; i++) {
    int y = startY + (i * rowHeight);
    drawStockRow(stocks[i], y);
    
    // Draw separator line between rows
    if(i < maxRows - 1) {
      tft.drawLine(0, y + rowHeight - 1, 320, y + rowHeight - 1, GRID_COLOR);
    }
  }
}

void drawStockRow(StockData &stock, int y) {
  // Symbol (left aligned)
  tft.setTextSize(1);
  tft.setTextColor(TEXT_WHITE, BG_COLOR);
  tft.setCursor(5, y + 2);
...
void drawSparkline(StockData &stock, int x, int y, int w, int h) {
